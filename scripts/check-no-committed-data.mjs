...
export const RULES = [
  ...
  },
  {
    ...
    test: (p) =>
      /(^|\/)\.env$/.test(p) ||
      (/(^|\/)\.env\.[^/]+$/.test(p) &&
        !/\.(example|sample|template)$/.test(p)),
  },
  {
    why: "code-signing material",
    test: (p) => /\.(p12|pfx|mobileprovision)$/.test(p),
  },
  {
    why: "SSH private key",
    test: (p) => /(^|\/)id_(rsa|ed25519|ecdsa|dsa)$/.test(p),
  },
];
...
function main() {
  ...
  if (hits.length > 0) {
    console.error(
      "::error::data-guard — refusing database/secret file(s) in the repository:",
    );
  ...
  console.log(
    `data-guard OK — scanned ${paths.length} path(s), no database/secret files.`,
  );
}
