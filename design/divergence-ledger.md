...
| F10 | Run detail                        | `213:3035`  | Figma            | `src/components/RunDetail.tsx`                                                                         | `src/components/RunDetail.test.tsx`, `e2e/run-detail.spec.ts`                                                          | —                                  | Partial (richer): timeline, logs, metrics, lineage, AI diagnosis exist; completed-task status is exposed to assistive tech (#239); the rerun path is now admission-controlled with a visible queued/admitted/duplicate outcome (#263/#264); the safe opt-in fix-agent **backend + CLOUD & LOCAL fix paths shipped disabled-by-default** (backend #275/#277/#278/#279/#281; cloud propose-only #284; local rerun-gated #286/#287/#288/#289; **PR2e Option C born-draft cloud #313**, §5d), no UI surface yet; per-step logs/copy + fix-agent action UI pending (D05) | Agent actions: safe backend + cloud/local fix paths shipped, disabled-by-default; UI pending (D05)                       | KP·Tabs | Pre-P1 — pending G04 audit |
...
- **Decision-3 (global queue-only) manual-run standardization — COMPLETE:** all
  ...
  **removed** (`#266`); `rerun_workflow` now routes through the admission choke
  ...
  (`#268`).
- **D05 (`F10` run-detail agent actions) — safe opt-in backend + CLOUD & LOCAL fix
  paths SHIPPED, disabled-by-default; UI pending:** the plan-first-reviewed contract
  for the run-detail "agent actions" ("Open in Cursor / Dispatch fix agent") is merged
  & green on `main` — the safety-primitive **backend foundation** across 5 PRs
  (`#275`/`#277`/`#278`/`#279`/`#281`), the **CLOUD** propose-only path (`#284`), the
  **LOCAL** rerun-gated path (`#286`/`#287`/`#288`/`#289`), and **PR2e "Option C"**
  (`#313`, §5d) — with the feature **disabled by default and no UI surface yet**. Every
  safety invariant carries a biting failing-first test: backend `open_url` scheme guard
  (allow only `http`/`https`/`cursor`, reject a leading `-`), symlink-safe path
  confinement anchored to the app `workspace_root`, prompt-fencing of untrusted
  `stderr` + secret non-leak (`error_analysis` dropped), a mandatory dispatch rate cap,
  a namespaced idempotency key, no-hijack of `rerun`/`backfill`, and a per-dispatch
  audit row. UI exposure (Settings "Cursor integration" section + RunDetail actions +
  consent `Modal`) is **pending**. **Operator decision (PROPOSE-ONLY):** the fix agent
  targets the REAL repo (production included) and a fix is **NEVER** auto-merged or
  auto-applied — there is no PR-merge code path anywhere, and the seam never sets
  `workOnCurrentBranch`, so the agent always pushes to a NEW branch. **PR2e "Option C"
  (`#313`) makes the CLOUD path race-free and REVERSES #284's `auto_create_pr=true`:**
  the **scheduler** (not the cloud agent) now opens the cloud fix PR **born `--draft`**
  — the config overlay forces the cloud agent's `auto_create_pr=false` and the
  scheduler runs `gh pr create -R <owner/repo> --draft --base main --head <branch>`
  with an app-authored title/body; a born-draft PR is auto-merge-**ineligible**, so no
  non-draft cloud fix PR can ever exist. This **UNIFIES the cloud path with the LOCAL
  path**, which isolates the agent in a throwaway git worktree, re-runs the failed job
  through admission control, and opens its own **draft** PR only on a passing rerun.
  Defense-in-depth (convert-to-draft fallback + orphaned-PR reconcile; injection-safe
  branch/slug validation; argv `Vec`, no shell) is fail-closed + operator-alerted
  (§5d). Every other guardrail (opt-in/consent, rate cap, hard-guard duplicate
  dispatch, audit, prompt-fence, path confinement, idempotency) is retained.
  Figma-mock-first then code.
...
| D05 Run-detail agent actions | in progress    | **Safe opt-in backend + CLOUD & LOCAL fix paths**, **shipped disabled-by-default** for `F10` "Open in Cursor/Dispatch fix agent" (backend #275/#277/#278/#279/#281; cloud #284; local #286/#287/#288/#289; **PR2e Option C #313**; failing-first tests per §1/§5d); UI pending. **PROPOSE-ONLY:** real repo (production allowed); NO PR-merge path + never sets `workOnCurrentBranch` (never auto-merged/applied, always a new branch). **PR2e "Option C" (#313) REVERSES #284:** the **scheduler** (not the cloud agent) now opens the cloud fix PR **born `--draft`** with the overlay forcing `auto_create_pr=false` ⇒ auto-merge-INELIGIBLE ⇒ **race-free**, unifying the cloud path with the local draft-PR path (§5d); other guardrails retained. Figma-mock-first |
...
### 5d. `D05` PR2e "Option C" — born-draft cloud fix — evidence (2026-07-13)

PR2e **"Option C"** (PR #313, merged `01fce96`; closes #298) makes the D05 **CLOUD**
fix path **race-free** by having the **scheduler** — not the cloud agent — open the
fix PR **born as a `--draft`**. It **REVERSES** #284's "the cloud agent opens the PR /
`auto_create_pr=true`" propose-only contract and **UNIFIES** the cloud path with the
LOCAL rerun-gated path (#286/#287/#288/#289), which already opens its own draft PR.

- **Decision + mechanism.** The config overlay now forces the cloud agent's
  `auto_create_pr=false`; the scheduler itself runs
  `gh pr create -R <owner/repo> --draft --base main --head <cursor-branch>` with an
  app-authored title/body. A born-`--draft` PR is **auto-merge-INELIGIBLE**, so there
  is **no window** in which a non-draft cloud fix PR can exist — **race-free**. An
  explicit `autoCreatePR: false` is now sent on the dispatch.
- **Defense-in-depth (all fail-closed + operator-alerted).** Two guards back the
  primary mechanism: a **convert-to-draft FALLBACK** — if a future Cursor build
  ignores `auto_create_pr=false` and returns a `pr_url`, the scheduler converts that
  PR to draft; and an **orphaned-PR RECONCILE** — if it opens a non-draft PR but omits
  `prUrl`, the scheduler's failed `gh pr create` triggers `gh pr list --head <branch>`
  → convert any found PR to draft. The branch and `owner/repo` slug are **strictly
  validated (injection-safe)** and argv is passed as a `Vec` (no shell).
- **Review provenance.** **Bugbot** (2 HIGH + 1 MEDIUM, all fixed + wired + tested) +
  an **independent Security Review** (clean, no medium+) + orchestrator delta
  verification; **406 cargo tests** + **clippy `-D warnings`** green; **`ci-required`
  green**. The feature stays **INERT** (behind the unbuilt `F10` UI).
