...
> **Status snapshot:** 2026-07-11 · off `origin/main`.
...
| F07 | Workflows list                    | `207:1303`  | Figma            | `src/components/WorkflowList.tsx`, `src/components/WorkflowCard.tsx`                                   | `src/components/WorkflowList.test.tsx`, `e2e/workflows-errors.spec.ts`                                                 | via `WorkflowCard`                 | Partial: env/frequency grouping + actions exist; compact rows, Active/Disabled filter, tz note, collapsible groups pending (unify in code, D04) | Delete action lives on list, not editor (D04 accept) | KP·Tabs | Pre-P1 — pending G04 audit |
| F08 | Workflow detail                   | `224:1376`  | Figma            | `src/components/WorkflowDetail.tsx`                                                                    | `src/components/WorkflowDetail.test.tsx`, `e2e/workflow-detail.spec.ts`                                                | —                                  | Partial/close: config, heatmap, recent runs, drill-downs exist; designed KPI set incomplete                                                     | — (DR02 redesign)                                    | KP·Tabs | Pre-P1 — pending G04 audit |
...
| F10 | Run detail                        | `213:3035`  | Figma            | `src/components/RunDetail.tsx`                                                                         | `src/components/RunDetail.test.tsx`, `e2e/run-detail.spec.ts`                                                          | —                                  | Partial (richer): timeline, logs, metrics, lineage, AI diagnosis exist; per-step logs/copy + fix-agent actions pending (D05)                    | Agent actions gated on safe backend (D05)            | KP·Tabs | Pre-P1 — pending G04 audit |
| F11 | Global History                    | `228:1303`  | Figma            | `src/components/GlobalHistory.tsx`                                                                     | `src/components/GlobalHistory.test.tsx`, `e2e/global-history.spec.ts`                                                  | —                                  | Partial; scope-expanded (D03): filters + drill-down exist; sticky Lookback/search/KPIs/env+duration cols + rehomed F06 charts pending           | Hosts rehomed F06 aggregate metrics (D03)            | KP·Tabs | Pre-P1 — pending G04 audit |
...
| F17 | Tray popup                        | `231:1303`  | shipped-behavior | `src/components/MenuBarPopup.tsx`                                                                      | `src/components/MenuBarPopup.test.tsx`, `e2e/popup.spec.ts`                                                            | —                                  | Partial/divergent: runtime 340×440 glance/control; Figma 384×590 with extra controls (D02)                                                      | Popup role/size 340×440 (D02, pending confirm)       | KP·Tabs | Pre-P1 — pending G04 audit |
| F18 | Workflow-scoped Run History       | `296:2863`  | Figma            | `src/components/RunHistory.tsx`                                                                        | `src/components/RunHistory.test.tsx`, `e2e/workflow-history.spec.ts`                                                   | —                                  | Partial: heatmap/table/rerun exist; sticky Lookback/search/KPIs pending                                                                         | — (DR02 redesign)                                    | KP·Tabs | Pre-P1 — pending G04 audit |
...
| C06 | InfoTip — Rest/Hover                                                            | `115:531`         | Figma            | `src/components/InfoTip.tsx`                                                                  | `src/components/InfoTip.test.tsx`                          | ✓ live | Implemented (**ahead of baseline**); hover/focus reveal; glossary rows code-only                                           | — (satisfies G09 InfoTip convention)                            | KP·DS   | Published lib — pending G04 audit |
...
| C32 | StatusDot — status-dot/mc-dot × Succeeded/Running/Failed/Queued/Warning         | `479:4257`        | repo-SOT         | `src/components/StatusDot.tsx`                                                                | `src/components/StatusDot.test.tsx`                        | ✓ live | Partial, CC; two bases mapped; status semantics/styles drift from design                                                   | Status semantics/styles (unify in code, D04)                    | KP·DS   | Published lib — pending G04 audit |
...
| D02 Popup role               | recommend      | Retain 340×440 glance-and-act (drives `F17` divergence)                                             |
