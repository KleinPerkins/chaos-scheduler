...
- **D05 (`F10` run-detail agent actions) — safe opt-in backend SHIPPED,
  ...
  **disabled by default and no UI surface yet**. Every safety invariant carries a
  ...
  fix agent now targets the REAL repo (production included). The app
  STRUCTURALLY guarantees two things: (a) a fix is NEVER auto-merged or
  auto-applied — there is no PR-merge code path anywhere, and the seam never
  sets `workOnCurrentBranch`, so the agent always pushes to a NEW branch and only
  opens a PR; and (b) the seam FORCES `auto_create_pr=true`. The opened PR being
  a **DRAFT** is Cursor Cloud's documented default for a programmatic dispatch —
  an accepted external dependency, NOT something this code byte-forces; a human
  always reviews + merges. This DROPS the earlier non-production / sandbox target
...
| D05 Run-detail agent actions | in progress    | **Safe opt-in backend**, **shipped disabled-by-default** for `F10` "Open in Cursor/Dispatch fix agent" (#275/#277/#278/#279/#281; failing-first tests per §1); UI pending. **Revised → PROPOSE-ONLY:** real repo (production allowed); app forces `auto_create_pr=true` and has NO PR-merge path + never sets `workOnCurrentBranch` (never auto-merged/applied, always a new branch), while the opened PR's **DRAFT** state is Cursor Cloud's documented default (accepted external dependency, not byte-forced); drops old non-prod/sandbox gate; other guardrails retained. Figma-mock-first |
