...
| F17 | Tray popup                        | `231:1303`  | shipped-behavior | `src/components/MenuBarPopup.tsx`                                                                      | `src/components/MenuBarPopup.test.tsx`, `e2e/popup.spec.ts`                                                            | —                                  | Semantic + visual parity landed at the retained 340×440 glance size (#237/#238): named Next Runs/Recent Results regions, admission-control "Queue run" (#237), an update aside (CTA labeled "Update"), a semantic footer, dark/light axe + a deterministic Linux baseline. No "Pause all"; the 384×590 mini-dashboard (D02) was not taken                                                        | Popup role/size 340×440 glance-and-act (D02); 384×590 mini-dashboard not taken | KP·Tabs | Pre-P1 — pending G04 audit |
| F18 | Workflow-scoped Run History       | `296:2863`  | Figma            | `src/components/RunHistory.tsx`                                                                        | `src/components/RunHistory.test.tsx`, `e2e/workflow-history.spec.ts`                                                   | —                                  | Partial: heatmap (keyboard-accessible cells, #244) + table + search/status filter + rerun exist; workflow-level manual run enters admission control (#229); async load failures announce via role=alert (#248); rows surface snapshotted run_environment (schema v13); sticky Lookback/KPIs pending                                                                                              | — (DR02 redesign)                                                              | KP·Tabs | Pre-P1 — pending G04 audit |
