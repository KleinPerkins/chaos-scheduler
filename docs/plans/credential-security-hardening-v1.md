...
- **DECISION REVERSED 2026-08-24 — Option B (envelope encryption) is now MANDATED and ON BY DEFAULT**, layered on top of the Option A (FileVault-stack / PR-C) file hardening. The earlier (2026-08-23) deferral is superseded. The full design, key lifecycle, rotation, and lost-key recovery posture are recorded in [ADR 0011](../adr/0011-envelope-encryption-secrets-at-rest.md).
  ...
  - **Option B — envelope encryption (this PR-E; DELIVERED).** Secret-bearing fields are AEAD-encrypted at rest (XChaCha20-Poly1305) under a 256-bit **DEK** that is itself wrapped by a 256-bit **KEK** held in the macOS Keychain (same trust boundary as the managed-key foundation, ADR 0010). Ciphertext is bound to its field location via AAD and stored as an idempotent `enc:v1:` token. A copied/backed-up DB becomes ciphertext, and same-user malware must also pull the ACL-bound Keychain KEK. Encrypt-on-write / decrypt-on-read sits at the single `db.rs` chokepoint, so the service and every adapter still see plaintext and the existing redaction runs above the decrypt.
- **DoD (met):** secret-bearing fields are AEAD-encrypted at rest under a Keychain-held KEK + wrapped DEK; the v19 migration is **transactional and idempotent** (encrypts existing plaintext in place, skips already-`enc:v1:` values); pre-migration `.bak` sidecars are securely wiped only after a verified encrypting sweep; KEK rotation (re-wrap) and DEK rotation (re-encrypt + version bump) are implemented and tested; a missing/unreadable master key degrades to a **secrets-locked** state (`__secret_unavailable__`, secret writes rejected, non-secret ops proceed) with a `reprovision_secrets` recovery path; every test injects `FakeKeyStore` and never touches the real Keychain.
...
- **PR-E (DELIVERED / MANDATED, on by default):** secret-bearing fields (SMTP password global+profiles, `inbound_webhook_secret`, workflow `spec_json`/`trigger_config`/`queue_config` secrets) AEAD-encrypted at rest as `enc:v1:` tokens under a Keychain-held KEK + wrapped DEK; v19 migration transactional + idempotent (encrypts existing plaintext in place, skips already-sealed); pre-migration backups securely wiped after a verified sweep; KEK + DEK rotation tested; missing/unreadable master key degrades to a secrets-locked graceful state (`__secret_unavailable__`, secret writes rejected, non-secret ops proceed) with a re-provision recovery path; decrypt-on-read keeps redaction + offboard-purge unchanged above the seam; all tests inject `FakeKeyStore`. See [ADR 0011](../adr/0011-envelope-encryption-secrets-at-rest.md).
