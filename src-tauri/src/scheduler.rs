...
impl WorkflowScheduler {
    pub fn new(db: Arc<Database>) -> Self {
        let email_enabled = db.get_email_config()
            .map(|c| c.enabled)
            .unwrap_or(false);
        Self {
            db,
    ...
    pub fn refresh_email_config(&self) {
        let enabled = self.db.get_email_config()
            .map(|c| c.enabled)
            .unwrap_or(false);
    ...
    pub fn find_due_workflows(&self) -> Vec<DueWorkflow> {
        ...
        for workflow in workflows {
            ...
            let Some(scheduled_time) = latest_scheduled_multi(&workflow.cron_schedule, tz, since, now) else {
                continue;
            };
            ...
            log::info!("Running due workflow: {} (scheduled for {})", workflow.name, scheduled_time);
...
pub fn execute_workflow(
    ...
    let run = db.create_run(&workflow.id)
        ...
        Ok(output) => {
            ...
            if let Some(pid) = bg_pid {
                ...
                std::thread::spawn(move || {
                    monitor_background_pid(pid, &run_id, &wf_name, &wf_script, &root, bg_log.as_deref(), email_enabled && wf_email, &db);
                });
            ...
            Ok(RunResult {
                ...
                script_path: workflow.script_path.clone(),
                success,
                should_notify: if success { notify_on_success } else { notify_on_failure },
                email_on_failure: workflow.email_on_failure,
            })
        }
        Err(e) => {
...
fn build_workflow_command(script_path: &str, chaos_labs_root: &str, python_path: &str) -> Command {
...
fn extract_result_url(stdout: &str) -> Option<String> {
    for line in stdout.lines().rev() {
        ...
        if trimmed.starts_with("https://docs.google.com/") || trimmed.starts_with("https://drive.google.com/") {
...
fn monitor_background_pid(pid: u32, run_id: &str, wf_name: &str, wf_script: &str, chaos_labs_root: &str, bg_log_path: Option<&str>, email_enabled: bool, db: &Database) {
    log::info!("Monitoring background PID {} for workflow '{}'", pid, wf_name);
    ...
    log::info!("Background PID {} for workflow '{}' has exited", pid, wf_name);
    ...
    let log_dir = std::path::Path::new(&log_path).parent().map(|p| p.to_string_lossy().to_string());
    let exit_code = log_dir.as_deref()
        .and_then(read_exit_status_from_dir)
        .or_else(|| read_exit_status_from_dir(&format!("{}/data/context-capture", chaos_labs_root)))
        .unwrap_or_else(|| {
...
fn recover_orphaned_runs(db: &Database, chaos_labs_root: &str) {
    ...
    log::info!("Found {} orphaned running run(s), recovering...", running.len());
    ...
    for run in &running {
        ...
        let result_url = extract_result_url(&best_stdout)
            .or_else(|| {
                if best_log_path.is_empty() { None } else { Some(format!("file://{}", best_log_path)) }
            });

        let exit_code = read_exit_status_from_dir(&format!("{}/data/context-refresh", chaos_labs_root))
        ...
        log::info!("Recovered orphaned run {} ({}) — finalized as {}", &run.id[..8], wf_name, status_label);
    }
}
            ...
            loop {
                ...
                for wf in &due {
                    match execute_workflow(
                        &db, &chaos_labs_root, &python_path, &wf.id,
                        notify_success, notify_failure, email_enabled,
                    ) {
    ...
    match crate::commands::run_email_script(chaos_labs_root, &config, Some(&run_context), "alert") {
        Ok(val) => {
            ...
            if success {
                ...
                log::warn!("Failed to send failure email for '{}': {}", result.workflow_name, error);
            }
        }
        Err(e) => {
            log::warn!("Email script invocation failed for '{}': {}", result.workflow_name, e);
        }
    }
}
...
mod tests {
    ...
    fn next_run_single_cron_backward_compat() {
        ...
        assert!(result.is_some(), "single cron should produce a next run time");
    }
    ...
    fn next_run_multi_mixed_field_counts() {
        ...
        assert!(result.is_some(), "mixed 5-field and 7-field should both parse");
    }
    ...
    fn next_run_double_semicolon() {
        ...
        assert!(result.is_some(), "double semicolon should skip empty segment");
    }

    #[test]
    fn next_run_no_whitespace_between() {
        let result = next_run_multi("0 5 * * *;0 13 * * *", chrono_tz::UTC);
        assert!(result.is_some(), "no whitespace after semicolon should work via trim");
    }
    ...
    fn next_run_one_invalid_subexpr() {
        ...
        assert!(result.is_some(), "invalid sub-expression should be skipped, valid ones used");
    }

    #[test]
    fn next_run_all_invalid() {
        let result = next_run_multi("invalid; also_invalid", chrono_tz::UTC);
        assert!(result.is_none(), "all invalid expressions should return None");
    }
    ...
    fn tz_aware_dst_transition() {
        ...
        let winter_base = chrono::NaiveDate::from_ymd_opt(2026, 1, 15).unwrap()
        ...
        let winter_next = {
            ...
            let next = schedule.after(&winter_utc.with_timezone(&ny)).next().unwrap();
            next.with_timezone(&Utc)
        };
        assert_eq!(winter_next.hour(), 14, "9 AM EST should be 14:00 UTC");

        // Pick a known summer date (Jul 15, 2026 — EDT, UTC-4)
        let summer_base = chrono::NaiveDate::from_ymd_opt(2026, 7, 15).unwrap()
            .and_hms_opt(0, 0, 0).unwrap();
        let summer_utc = Utc.from_utc_datetime(&summer_base);
        let summer_next = {
            let normalized = normalize_cron(cron);
            let schedule = cron::Schedule::from_str(&normalized).unwrap();
            let next = schedule.after(&summer_utc.with_timezone(&ny)).next().unwrap();
            next.with_timezone(&Utc)
        };
    ...
    fn latest_scheduled_single_cron() {
        ...
        assert!(result.is_some(), "daily cron should have a recent scheduled time");
    }
    ...
    fn latest_scheduled_three_expressions() {
        let now = Utc::now();
        let since = now - chrono::Duration::days(2);
        let result = latest_scheduled_multi("0 5 * * *; 0 12 * * *; 0 17 * * *", chrono_tz::UTC, since, now);
    ...
    fn latest_scheduled_tz_aware() {
        ...
        assert!(result.is_some(), "daily 9am Chicago should have a recent fire time within 2 days");
        let t = result.unwrap();
        // 9 AM Chicago in winter is 15:00 UTC, in summer 14:00 UTC
        assert!(t.hour() == 14 || t.hour() == 15,
            "9 AM Chicago should map to UTC 14 or 15 depending on DST, got {}", t.hour());
    }
    ...
    fn latest_scheduled_one_invalid() {
        let now = Utc::now();
        let since = now - chrono::Duration::days(2);
        let result = latest_scheduled_multi("0 5 * * *; garbage; 0 17 * * *", chrono_tz::UTC, since, now);
        assert!(result.is_some(), "should skip invalid and use valid sub-expressions");
    }
    ...
    fn multi_schedule_later_expr_still_triggers() {
        ...
        let latest = latest_scheduled_multi("0 5 * * *; 0 17 * * *", chrono_tz::UTC, since, today_5am);
        ...
        let latest_after = latest_scheduled_multi("0 5 * * *; 0 17 * * *", chrono_tz::UTC, since, at_5pm_plus);
        assert!(latest_after.is_some());
        assert!(latest_after.unwrap() > today_5am,
